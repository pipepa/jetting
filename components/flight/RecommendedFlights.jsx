...
export default function RecommendedFlights() {
  ...
  async function fetchData() {
    ...
    if (newFetchedData.length == 0) {
      try {
        const countryPlaces = await getPlaces({ searchTerm: nearestFlightLocation.country.name })
          ...
          {paginatedSelectedRecommendedFlights.length == 0 && isLoading ? (
            <div className="loading-overlay">
              ...
              <span className="text-light-1 mt-10">
                Affordable flights near you are loading ...
              </span>
            </div>
          ) : (
          <>
            ...
            {defaultLocationUsed && (
              <div className="w-100 text-center mb-40">
                <span className="border-dashed rounded-20 px-22 py-15 fw-600">
                  We could not find any flights from {nearestFlightLocation?.city?.name} to {selectedTab.name}. Showing from {defaultLocationUsed?.city?.name}
                </span>
              </div>
            )}

            <div className="row">
              <PromoFlights recommendedFlights={paginatedSelectedRecommendedFlights} />
            </div>
            ...
            <div className="row y-gap-20 justify-between items-center mt-30 mb-10">
              <Providers />
            </div>
