...
const FlightPropertyItem = ({
  ...
  const { formattedDate: formattedDepartureDate, formattedTime: formattedDepartureTime } = formatDateTime(departureDatetime, false, ['month', 'day'])
  const { formattedDate: formattedArrivalDate, formattedTime: formattedArrivalTime } = formatDateTime(arrivalDatetime, false, ['month', 'day'])

  const duration = formatMinutes(durationInMinutes)
    ...
    <>
      ...
      <div className="row y-gap-10 x-gap-20 sm:x-gap-10 mr-0 items-top">
        <div className="col-3 col-md-3 d-flex flex-column justify-between">
          ...
          <div className="text-12 lh-12 text-light-1">{departureName}</div>
          <div className="text-12 lh-16 text-light-1">{formattedDepartureDate}</div>
        </div>

        <div className="col">
          <div className="text-center text-12 mb-10">{duration}</div>
          <ul className="flightLineCard text-center border-top-light d-flex border-width-1 align-items-start justify-content-between">
            <li className="flight-stop flight-start">
              ...
              <i className={`flight-icon icon-aircraft-take-off text-18 text-rich-300`} />
            </li>
            ...
            {stops.length > 0 && (
              ...
              {stops.map((item, index) => {
                const stopTime = getSkyscannerDateTimeDifference(item.arrivalDateTime, item.departureDateTime)
                return (
                  <Popup
                    key={parentItemKey + item.entityId + index}
                    trigger={
                      <li className="flight-stop flight-between">
                        <span className={`stop-name sm:d-none`}>
                          {item.originPlace.iata}
                          {item.transferPlace?.iata && ` - ${item.transferPlace.iata}`}
                        </span>
                        <div className="stop-decor"></div>
                      </li>
                    }
                  ...
                  >
                    ...
                    >
                      {item.transferPlace ? (
                        <>
                          Stop: {item.originPlace.name} - {item.transferPlace.name}
                          <br />
                          {stopTime}
                          <br />
                          <span className="text-red-1">
                            Note: Arrival and departure at different airports require airport transfers!
                          </span>
                        </>
                      ) : (
                        <>
                          Stop: {item.originPlace.name}
                          <br />
                          {stopTime}
                        </>
                      )}
            ...
            <li className="flight-stop flight-finish">
              ...
              {/* <img className="flight-icon" src="/img/flights/flight-land.svg" alt={t('flights.flight_land')} /> */}
                <i className={`flight-icon icon-aircraft-landing text-18 text-rich-300`} />
            </li>
          </ul>
          <div className={`mt-10 stop-name text-blue-1 text-center ${stops.length > 0 ? 'd-none sm:d-block' : ''}`}>
            {stops.length > 0 ? (
              <>
              {stops.length} {pluralize(stops.length, 'Stop', 'Stops')}
              </>
            ) : (
              <>
              Nonstop
        ...
        <div className="col-3 col-md-3 text-right d-flex flex-column justify-between">
          ...
          <div className="text-12 lh-12 text-light-1">{arrivalName}</div>
