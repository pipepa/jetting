    ...
    <>
      <div className="row y-gap-10 justify-between items-center">
        <div className="col-auto">
          ...
          {name === "Children" ? (
            <div className="text-14 lh-12 text-light-1 mt-5">Age 2 - 15</div>
          ) : name === "Infants" && (
            <div className="text-14 lh-12 text-light-1 mt-5">Age 0 - 2</div>
          )}
        </div>
        {/* End .col-auto */}
        <div className="col-auto">
    ...
    <div className="searchMenu-guests px-24 lg:py-20 md:py-10 lg:px-0 js-form-dd js-form-counters">
      ...
      >
        ...
        <div className="text-15 text-dark-1 ls-2 lh-18" style={{minWidth: '146px'}}>
          <span className="js-count-adult">{passengerCounts.Adults}</span> {pluralize(passengerCounts.Adults, 'adult', 'adults')} {" "}
            {passengerCounts.Children ? (
              <>
              - {" "} <span>{passengerCounts.Children}</span> {pluralize(passengerCounts.Children, 'child', 'children')} {" "}
              </>
            ) : ''}
            {passengerCounts.Infants ? (
              <>
                - {" "} <span>{passengerCounts.Infants}</span> {pluralize(passengerCounts.Infants, 'infant', 'infants')}
              </>
            ) : ''}
        </div>
      </div>
