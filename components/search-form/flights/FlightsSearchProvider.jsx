...
import { makeSearch, generateSearchUrl } from "@/modules/skyscanner/utils/search"
...
import { getHotelsLink } from "@/modules/vio/utils/search"
...
export const FlightsSearchProvider = ({ children }) => {
  ...
  const [storredFlightsSearchData, setStorredFlightsSearchData] = useLocalStorage("searchData", {})
  ...
  const fetchAndSetPlaces = async ({ fromPlace, toPlace }) => {
    ...
    if (fromPlace?.iataCode && !fromPlace?.name) {
      const places = await getPlaces({ searchTerm: fromPlace.iataCode })
  ...
  //filters
  const [tripType, tripTypeOLD] = useState("Round Trip") //flightsSearchData?.tripType ?? "Round Trip"
  //always show round trip
  const [tripTypeNEW, setTripType] = useState(flightsSearchData?.tripType ?? "Round Trip")
  ...
  //form submit
  const handleFlightsSearchFormSubmit = async ({ samePage, scrollToResults = true }) => {
    const searchData = {
      fromPlace,
      toPlace,
      passengerCounts,
    ...
    if (!hasErrors) {
      ...
      if (hotelsSearch == 'true') {
        ...
        try {
          const url = await getHotelsLink({ searchData })
